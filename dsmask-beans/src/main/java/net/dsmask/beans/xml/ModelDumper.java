...
public class ModelDumper extends XmlNames {
    ...
    public Element dump(ModelEntity me) {
        switch (me.getEntityType()) {
            ...
            case Config:
                return dumpConfig((MaskingConfig) me);
        }
        throw new IllegalArgumentException(me.toString());
    }

    public void dump(ModelPackage mp, XmlObjectDeployer xod) {
        for (EntityType et : EntityType.values()) {
            for (ModelName mn : mp.list(et)) {
                ModelEntity me = mp.find(mn);
                if (me!=null)
                    xod.save(mn, dump(me));
            }
        }
    ...
    private Element dumpConfig(MaskingConfig x) {
        Element el = elementEntity(x);
        for ( MaskingProfile p : x.getProfiles()) {
            Element elProfile = dumpProfile(p);
            if (elProfile!=null)
                el.addContent(elProfile);
        }
        return el;
    }
