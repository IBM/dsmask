    ...
    public MetaTable findTable(String db, String schema, String table) {
        for (PackageAccessor pa : data) {
            for (ModelName name : pa.list(EntityType.Metadata)) {
                ...
                MetaTable mt = mm.getTable();
                if (mt.isIgnoreCase()) {
                    if (!ModelUtils.equalsCI(mt.getTableName(), table))
                        continue;
                    if (!ModelUtils.equalsCI(mt.getSchemaName(), schema))
                        continue;
                    if (!ModelUtils.equalsCI(mt.getDatabaseName(), db))
                        continue;
                } else {
                    if (!ModelUtils.safeEquals(mt.getTableName(), table))
                        continue;
                    if (!ModelUtils.safeEquals(mt.getSchemaName(), schema))
                        continue;
                    if (!ModelUtils.safeEquals(mt.getDatabaseName(), db))
                        continue;
                }
                return mt;
            }
