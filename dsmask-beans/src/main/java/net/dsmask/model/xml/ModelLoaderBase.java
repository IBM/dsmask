 ...
 * Generic implementation misses the retrieveRules() operation.
 * @author zinal
 */
public abstract class ModelLoaderBase extends XmlNames implements ModelAccessor {

    protected final XmlProvider provider;
    private final Deque<XmlObject> stack = new ArrayDeque<>();
    private final Map<EntityType, Map<String, ModelEntity>> pool = new HashMap<>();

    public ModelLoaderBase(XmlProvider provider) {
    ...
    protected ModelEntity findInPool(EntityType et, String name) {
        Map<String, ModelEntity> m = pool.get(et);
        if (m==null)
            return null;
        return m.get(ModelUtils.lower(name));
    }

    protected void addToPool(ModelEntity me) {
        Map<String, ModelEntity> m = pool.get(me.getEntityType());
        if (m==null) {
            m = new HashMap<>();
            pool.put(me.getEntityType(), m);
        }
        m.put(me.getId(), me);
    }
    ...
    public ModelEntity find(EntityType type, String name) {
        ModelEntity me = findInPool(type, name);
        if (me==null) {
            XmlObject xo = provider.getObject(type, name);
            if (xo==null)
                return null;
            handleEntity(xo);
            me = findInPool(type, name);
        }
        return me;
    }
    ...
    private void handleProfile(XmlObject xo) {
        MetaEntity me = resolve(EntityType.Metadata, xo.getElement(), ATT_TABLE);
        MetaTable table = me.getTable();
        MaskingProfile mp = new MaskingProfile(table, xo.getName());
        for (Element elOp : xo.getChildren(TAG_Operation)) {
    ...
    public boolean exists(ModelName mn) {
        if (mn==null)
            return false;
        if (findInPool(mn.getEntityType(), mn.getId()) != null)
            return true;
        return provider.exists(mn);
    }

    @Override
    public boolean exists(EntityType type, String name) {
        if (findInPool(type, name) != null)
            return true;
        return provider.exists(type, name);
    }
